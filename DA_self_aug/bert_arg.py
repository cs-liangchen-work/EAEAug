...
def generate_input(tem_seq_in, tem_all_sentence_1_role, tem_segment_embedding_1_role):
    for seq_in_ in tem_seq_in:
        ...
        for event in m['gold_evt_links']:
            ...
            seq_in_list_tem_1 = []
            for list_ in m['sentences']:
                seq_in_list_tem_1 += list_
            for i in range(0, len(seq_in_list_tem_1)):
                if i >= role_begin and i <= role_end:
                    seq_in_list_tem_1[i] = '[MASK]'
            # seg
            seq_seg_1 = []
            for i in range(0, len(seq_in_list_tem_1)):
                seq_seg_1.append(0)
            for i in range(m['evt_triggers'][0][0], m['evt_triggers'][0][1] + 1):
                seq_seg_1[i] = 1
            seq_seg_1 = [0] + [1] + [0] + seq_seg_1 + [0]
            # 调整。
            seq_seg_1.insert(role_begin + 3, 0)
            seq_in_list_tem_1.insert(role_begin, '[MASK]')
            # role-prompt
            inputs_1 = '[CLS] ' + role + ' [SEP] ' + (' '.join(seq_in_list_tem_1)) + ' [SEP]'
            assert len(inputs_1.split()) == len(seq_seg_1)
            tem_all_sentence_1_role.append(inputs_1)
            tem_segment_embedding_1_role.append(seq_seg_1)
            # ===== n-1 =====
            if role_begin == role_end:
                continue
            # mask
            seq_in_list_tem_2 = []
            for list_ in m['sentences']:
            ...
            # 调整。
            del seq_seg_2[role_begin + 3]
            del seq_in_list_tem_2[role_begin]
            # role-prompt
