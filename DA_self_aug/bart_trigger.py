...
# ================================================ 数据读取 ============================================
def generate_input(tem_seq_in, tem_all_sentence_1_role, tem_segment_label_1_role, tem_segment_embedding_1_role):
    for seq_in_ in tem_seq_in:
        ...
        for event in m['gold_evt_links']:
            ...
            seq_label_2 = []
            for i in range(0, len(seq_in_list_tem_2)):
                seq_label_2.append(0)
            for i in range(role_begin, role_end + 1):
                seq_label_2[i] = 1
            seq_label_2 = [0] + [0] + [0] + seq_label_2 + [0]

            # 调整。
            for _ in range(0, trigger_end - trigger_begin):
                del seq_seg_2[trigger_begin + 4]
                del seq_label_2[trigger_begin + 4]
                del seq_in_list_tem_2[trigger_begin + 1]
            ...
            tem_segment_label_1_role.append(seq_label_2)
...
train_segment_label_1_role = []
train_segment_embedding_1_role = []
generate_input(train_seq_in, train_all_sentence_1_role, train_segment_label_1_role, train_segment_embedding_1_role)
...
for i in range(0, len(train_all_sentence_1_role)):
    ...
    print(inputs)
    print(train_segment_embedding_1_role[i])
    ...
    for j in range(0, len(predictions_list)):
        m = {}
        m['seq_in_mask'] = train_all_sentence_1_role[i]
        m['seq_in'] = train_all_sentence_1_role[i].replace('<mask>', predictions_list[j])
        m['segment_embed'] = train_segment_embedding_1_role[i]
        m['label'] = train_segment_label_1_role[i]
