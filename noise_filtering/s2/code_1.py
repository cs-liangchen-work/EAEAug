...
temF1 = 0.0
data_rate = 0.05
file_name = 'ee_da/my_denoise/dy_0.05/s1_pre5000/model_2/static_dict3_22.88797915891329.pkl'
...
with open('./data_select.txt', encoding='utf-8') as f:
    role_change = [line.strip() for line in f.readlines()]
...
for tem in train_mlm:
    ...
    for i in range(role_begin, role_end + 1):
        ...
        label[i] = 1
    if role_change[role_change_index] == '0':
        role_change_index += 1
    else:
        role_change_index += 1
...
model.load_state_dict(torch.load(str(file_name)))
