...
seed = 10
random.seed(seed)
random.shuffle(train_seq_in_tem)
'''
...
with open('./RAMS_1.0/data/test.jsonlines', encoding='utf-8') as f:
...
delete_list = [
    'the home of the iraqi microbiologist known as dr. germ. the woman who ran iraq\'s secret biological warfare laboratory',
    'rescued p.o.w. jessica lynch']
...
with open('./fn1.7.conll', encoding='utf-8') as f:
    ...
    if tem_seq == '':
        ...
        for i in range(0, len(sentence_token)):
            if sentence_label[i].split('-')[0] == 'S':
                ...
                bert_seq_in_ = '[CLS] ' + sentence_label[i].split('-')[1] + ' [SEP] ' + (
                    ' '.join(sentence_token)) + ' [SEP]'
                train_label.append([0] + [0] + [0] + train_label_ + [0])
                train_segment_embedding.append([0] + [1] + [0] + seq_seg + [0])
                train_all_sentence.append(bert_seq_in_)
...
for j in range(len(train_segment_embedding_1)):
    ...
    if len(i) < max_padding_len:
        ...
        train_segment_embedding_1[j] = train_segment_embedding_1[j][0:max_padding_len - 1] + [
            train_segment_embedding_1[j][-1]]
for j in range(len(dev_segment_embedding_1)):
    ...
    if len(i) < max_padding_len:
        ...
        dev_segment_embedding_1[j] = dev_segment_embedding_1[j][0:max_padding_len - 1] + [
            dev_segment_embedding_1[j][-1]]
for j in range(len(test_segment_embedding_1)):
    ...
    if len(i) < max_padding_len:
        ...
        test_segment_embedding_1[j] = test_segment_embedding_1[j][0:max_padding_len - 1] + [
            test_segment_embedding_1[j][-1]]
...
dev_loader = DataLoader(dataset=dev_set, batch_size=24, shuffle=False)
